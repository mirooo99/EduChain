  ...
  return (
    <div style={{ padding: '20px', backgroundColor: 'white', borderRadius: '8px' }}>
      ...
      <form onSubmit={handleVerify} style={{ display: 'flex', flexDirection: 'column' }}>
        <input type="text" placeholder="0x..." value={certId} onChange={(e) => setCertId(e.target.value)} required />
        <button type="submit" disabled={loading}>{loading ? 'Проверка...' : 'Провери'}</button>
      </form>
